...
export const GuaranteeQuote = () => {
    ...
    const fetchUserDetails = async () => {
        try {
            ...
            // Autofill holder data with user details
            setUserData(prev => ({
                ...prev,
                step2: {
                    ...prev.step2,
                    ...
                    holder_contact_person: userDetails.first_name + " " + userDetails.last_name || ""
                },
                step3: {
                    ...prev.step3,
    ...
    const handleQuestionChange = (questionId, answer, textarea = "") => {
        setUserData(prevData => ({
            ...
            step6: {
                ...prevData.step6,
                questions: prevData.step6.questions.map(q =>
    ...
    const isStepValid = (step) => {
        ...
        if (step === 5) { // Step 6 (questions)
            return stepData.questions.every(q => q.answer !== "");
        }

        // For other steps, check if all required fields are filled
        const requiredFields = Object.keys(stepData).filter(key =>
            key !== 'type_other_desc' && key !== 'holder_identification' &&
            key !== 'holder_contact_person' && key !== 'beneficiary_contact_person'
        );
    ...
    const handleSubmit = async () => {
        try {
            ...
            // Store data in localStorage for the proceed page
            localStorage.setItem('guaranteeData', JSON.stringify(requestData));

            // Navigate to proceed page
        ...
        <main>
            ...
            <section className="border-t-2 mx-5 md:mx-0 my-2 flex flex-col justify-center items-center">
                <div className="flex flex-col justify-center items-center my-10">
                    ...
                    ${index === currentStep ? "ml-[50px] md:ml-16" : ""}`}
                        ...
                        <span
                            ...
                            style={{
                                left: `calc(${currentStep * (currentStep === 5 ? 21 : 19.5)}%)`, // Adjusts plane's position dynamically based on the current step
                            }}
                        >
                ...
                <div className="my-5 flex flex-wrap justify-center items-center gap-5 w-full">
                    {/* Step 1 - Guarantee Type and Dates */}
                    {currentStep === 0 && (
                        <div className="flex flex-col justify-center items-center gap-5">
                            ...
                            <TravelSelect
                                ...
                                options={apiData.type_of_guarantees.map(t => ({ value: t.id, label: t.name }))}
                            />

                            {userData.step1.type_of_guarantee === "other" && (
                                <TravelInput
                                    type="text"
                                    placeholder="Type other description"
                    ...
                    {currentStep === 1 && (
                        <div className="flex flex-col justify-center items-center gap-5 sm:gap-10 w-full">
                            ...
                            <TravelSelect
                                ...
                                options={apiData.holder_types.map(t => ({ value: t.id, label: t.name }))}
                            />
                            ...
                            <TravelInput
                                type="text"
                                placeholder="Holder Identification"
                            ...
                            <TravelInput
                                type="text"
                                placeholder="Holder Contact Name"
                        ...
                        <section>
                            ...
                            {apiData.questions.map((question, index) => {
                                ...
                                return (
                                    <div key={question.id}>
                                        <ol start={index + 1}>
                                            <li className="max-w-[454px] mx-auto text-sm text-left list-decimal"
                ...
                <div className="flex justify-center items-center gap-3 vsm:gap-10 my-5">
                    ...
                    />
                    <ActionButton
                        text={currentStep < totalSteps - 1 ? t('guarantee_quote_page.buttons.next') : t('guarantee_quote_page.buttons.submit')}
                        iconPosition="right"
                        onClick={handleNext}
                        isNext
                    />
...
const TravelSelect = ({ placeholder, value, onChange, options, isInvalid }) => (
    ...
    >
        ...
        {options.map((option, idx) => (
            <option key={idx} value={option.value || option} className="font-semibold">
                {option.label || option}
            </option>
        ))}
    </select>
